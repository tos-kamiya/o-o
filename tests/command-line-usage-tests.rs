...
mod test {
    ...
    fn write_and_wait(file_path: &str, content: &str) -> std::io::Result<()> {
        let mut file = File::create(file_path)?; // File::create が返すResultを処理
        write!(file, "{}", content)?; // write!マクロを使用してファイルに書き込み、エラーを処理
        file.flush()?; // flush() が返すResultを処理
    ...
    fn capture_stdout_and_stderr() -> Result<(), io::Error> {
        ...
        write_and_wait(SU(&script), "echo \"stdout\" >&1\necho \"stderr\" >&2\n")?;
